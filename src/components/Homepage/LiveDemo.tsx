...
import { CheckCircle2, Layout, Utensils, Wallet, Target, HeartPulse } from 'lucide-react';
import { motion } from 'framer-motion';
import { fadeInVariants, staggerContainerVariants, slideInLeftVariants } from '../../theme/motion';
...
function LiveDemo() {
  ...
  useEffect(() => {
    ...
    const render = () => {
      ...
      particles.forEach(p => {
        ...
        p.y = (p.y + p.vy + 1) % 1;
        ctx.beginPath();
        ctx.arc(p.x * state.current.width, p.y * state.current.height, p.size, 0, Math.PI * 2);
      ...
      nodes.forEach((node, index) => {
        ...
        // Add floating effect
        const floatOffset = Math.sin(state.current.animationTime + index * 0.5) * 3;
        ...
        ctx.translate(0, -5);
        ctx.scale(1.1, 1.1);
        ctx.strokeStyle = t >= 0.5 ? 'rgba(255, 255, 255, 0.95)' : 'rgba(0, 163, 255, 0.95)';
        ctx.lineWidth = 3.5;
        ctx.lineCap = 'round';
        ctx.lineJoin = 'round';
        ...
        ctx.fillStyle = t >= 0.5 ? 'rgba(255, 255, 255, 0.9)' : 'rgba(0, 163, 255, 0.9)';
  ...
  const handleMouseDown = (e: React.MouseEvent | React.TouchEvent) => {
    ...
    const distHub = Math.hypot(x - state.current.hub.x, y - state.current.hub.y);
  ...
  const handleMouseMove = (e: React.MouseEvent | React.TouchEvent) => {
    ...
    for (const node of state.current.nodes) {
      const floatOffset = Math.sin(state.current.animationTime + state.current.nodes.indexOf(node) * 0.5) * 3;
  ...
  return (
    <div className="container padding-vert--xl">
      ...
      >
        ...
        <div className={styles.splitVisual} ref={containerRef} style={{ minHeight: '600px', flex: 2 }}>
          <canvas
            ref={canvasRef}
